...
exports.runComplexHardeningSuite = runComplexHardeningSuite;
...
async function runComplexHardeningSuite() {
    console.log('\n===============================================================');
    ...
    // ═════════════════════════════════════════════════════════════════════════
    {
        ...
        try {
            ...
            const replannedSteps = await planner.replan('Complete checkout order', { stepNumber: 1, description: 'Click checkout', toolName: 'browser.click', parameters: { selector: '#broken_checkout_selector' }, status: 'FAILED' }, 'Element #broken_checkout_selector not found in DOM after 4000ms', {
                currentUrl: preSnap.url,
                pageTitle: preSnap.title,
                compactSnapshot: accessibility_tree_js_1.AccessibilityTreeFormatter.toCompactString(preSnap.elements),
    ...
    // ═════════════════════════════════════════════════════════════════════════
    {
        ...
        try {
            ...
            const snap = await perception.getSnapshot();
            rep.observedWebpage = snap.url;
            const formatted = accessibility_tree_js_1.AccessibilityTreeFormatter.toCompactString(snap.elements);
...
if (typeof window !== 'undefined') {
    window.runComplexHardeningSuite = runComplexHardeningSuite;
}
//# sourceMappingURL=complex-web-hardening.js.map