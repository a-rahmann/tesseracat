...
exports.runLatencyBenchmarkSuite = runLatencyBenchmarkSuite;
...
const performance_profiler_js_1 = require("../agent/performance-profiler.js");
const voice_manager_js_1 = require("../voice/voice-manager.js");
async function runLatencyBenchmarkSuite() {
    const reports = [];
    const runtime = agent_runtime_js_1.AgentRuntime.getInstance();
    ...
    const executeBenchmarkCommand = async (command, timeoutMs = 4000) => {
        let completed = false;
    ...
    console.log('\n--- Running Test 5: Standalone Browser Micro-Controls ---');
    {
        ...
        for (const cmd of commands) {
            ...
            if (actionMs > maxFirstAction)
                maxFirstAction = actionMs;
            if (actionMs > 300)
                allPassed = false;
            await wait(100);
        }
...
//# sourceMappingURL=latency-benchmark.js.map