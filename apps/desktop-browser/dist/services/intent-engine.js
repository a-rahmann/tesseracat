...
class IntentEngine {
    ...
    classify(rawTranscript) {
        ...
        if (inNewTab)
            fallbackIntent.inNewTab = true;
        this.recordIntent(fallbackIntent);
        return fallbackIntent;
    }
    matchBrowserControl(text, rawText, cleanText, inNewTab = false) {
    ...
    matchOrdinalSelection(text, rawText, cleanText) {
        const ordinalMap = {
    ...
    matchMediaPlayback(text, rawText, cleanText, inNewTab = false) {
    ...
    matchNavigation(text, rawText, cleanText, inNewTab = false) {
    ...
    matchSearch(text, rawText, cleanText, inNewTab = false) {
    ...
    matchShopping(text, rawText, cleanText) {
    ...
    matchComparison(text, rawText, cleanText) {
    ...
    createFallbackIntent(text, rawText, cleanText) {
    ...
    recordIntent(intent) {
        ...
        if (intent.targetUrl)
            console.log(`[Intent] target: ${intent.targetUrl}`);
        if (intent.query)
            console.log(`[Intent] query: "${intent.query}"`);
        if (intent.action)
            console.log(`[Intent] action: ${intent.action}`);
        this.context.lastIntent = intent;
        if (intent.query)
            this.context.lastQuery = intent.query;
