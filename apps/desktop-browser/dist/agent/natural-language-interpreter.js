...
exports.NaturalLanguageInterpreter = void 0;
...
class NaturalLanguageInterpreter {
     ...
     */
    async interpret(rawText, currentUrl, currentTitle) {
        ...
        // 0. Gboard-Style Acoustic Intent Auto-Correction & Repair
        const autoCorrect = this.fuzzyAutoCorrect(cleanRaw);
        if (autoCorrect.wasRepaired) {
            console.log(`[NaturalLanguageInterpreter] Gboard-style intent repair: "${cleanRaw}" -> "${autoCorrect.corrected}" (${autoCorrect.reason})`);
        }
        // Strip wake phrases and conversational preambles
        const stripped = this.cleanWakeAndPreambles(autoCorrect.corrected);
        // Collect recent context
        const convTurns = conversation_manager_js_1.ConversationManager.getInstance().getRecentTurns(4);
        const recentContext = convTurns
            .map((t) => `${t.speaker === 'user' ? 'User' : 'Assistant'}: "${t.text}"`)
            .join('\n');
        const browserStore = browser_state_store_js_1.BrowserStateStore.getInstance();
        const activeTab = browserStore.getActiveTab();
        const rawUrl = typeof currentUrl === 'string' ? currentUrl : currentUrl?.currentUrl || currentUrl?.url;
        const rawTitle = typeof currentTitle === 'string' ? currentTitle : currentUrl?.currentTitle || currentUrl?.title;
        ...
        if (correctionMatch) {
            ...
            const lastGoal = context_manager_js_1.ContextManager.getInstance().getLastChainStep()?.goal || 'previous task';
            const rule = learned_rules_store_js_1.LearnedRulesStore.getInstance().recordUserCorrection({
                domain,
                pattern: stripped,
        ...
        if (fastPathGoal) {
            fastPathGoal = context_manager_js_1.ContextManager.getInstance().optimizeAndPrune(fastPathGoal, activeUrl);
        ...
        const domain = activeUrl && !activeUrl.startsWith('about:') ? new URL(activeUrl).hostname.replace(/^www\./, '') : undefined;
        const learnedRulesPrompt = learned_rules_store_js_1.LearnedRulesStore.getInstance().formatRulesPrompt(domain, stripped);
...
Output strictly valid JSON matching this schema:
{
  ...
  "confidence": number
     ...
     */
    detectFastPathIntent(cleanText, activeUrl) {
        const text = cleanText.toLowerCase().trim();
        if (!text)
            return null;
        // 0. CHECK PERSISTENT LEARNED RULES (User Corrections & Self-Healing Workarounds)
        const domain = activeUrl && !activeUrl.startsWith('about:') ? new URL(activeUrl).hostname.replace(/^www\./, '') : undefined;
        const applicableRules = learned_rules_store_js_1.LearnedRulesStore.getInstance().getApplicableRules(domain, text);
        ...
        if (playOrdinalMatch) {
            ...
            if (rawIdx === 'first' || rawIdx === '1st' || rawIdx === '1')
                index = 1;
            else if (rawIdx === 'second' || rawIdx === '2nd' || rawIdx === '2')
                index = 2;
            else if (rawIdx === 'third' || rawIdx === '3rd' || rawIdx === '3')
                index = 3;
            else if (rawIdx === 'fourth' || rawIdx === '4th' || rawIdx === '4')
                index = 4;
            else if (rawIdx === 'fifth' || rawIdx === '5th' || rawIdx === '5')
                index = 5;
            return {
                rawUserText: cleanText,
     ...
     */
    fallbackSemanticInterpreter(rawText, cleanText, activeUrl) {
     ...
     */
    fuzzyAutoCorrect(text) {
        const res = voice_grammar_corrector_js_1.VoiceGrammarCorrector.getInstance().correct(text);
        return {
    ...
    fallbackCategory(text) {
        const lower = text.toLowerCase();
        if (lower.includes('compare') || lower.includes('buy') || lower.includes('price'))
            return 'SHOPPING_COMPARISON';
        if (lower.includes('research') || lower.includes('study') || lower.includes('find out'))
            return 'RESEARCH';
        if (lower.includes('message') || lower.includes('dm') || lower.includes('chat'))
            return 'SOCIAL_COMMUNICATION';
        if (lower.includes('pdf') || lower.includes('paper'))
            return 'DOCUMENT_ANALYSIS';
        if (lower.includes('play') || lower.includes('video') || lower.includes('song'))
            return 'MEDIA_CONTROL';
        if (/^(?:open|go\s+to|visit)\s+/i.test(lower))
            return 'NAVIGATION';
        return 'GENERAL_AUTOMATION';
    }
}
exports.NaturalLanguageInterpreter = NaturalLanguageInterpreter;
//# sourceMappingURL=natural-language-interpreter.js.map