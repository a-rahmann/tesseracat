...
const capability_router_js_1 = require("./capability-router.js");
const goal_graph_js_1 = require("./goal-graph.js");
...
class AgentRuntime {
     ...
     */
    async retryActiveTaskWithRecovery(userCorrection) {
        ...
        if (userCorrection) {
            ...
            learned_rules_store_js_1.LearnedRulesStore.getInstance().recordUserCorrection({
                domain,
                pattern: targetGoal,
    ...
    async handleUserCommand(commandInput) {
        ...
        if (fastPathGoal && fastPathGoal.isFastPath) {
            ...
            if (fastPathGoal.fastPathAction === 'NAVIGATE' && fastPathGoal.suggestedTargetUrl) {
                ...
                if (fastPathGoal.entities.platform === 'YouTube Music' && fastPathGoal.suggestedTargetUrl) {
                    browser_automator_js_1.BrowserAutomator.getInstance().navigate(fastPathGoal.suggestedTargetUrl).then(async () => {
                        await media_controller_js_1.MediaController.getInstance().play();
                    }).catch(() => { });
                }
                    ...
                    if (!isRandomOrGeneric && query) {
                        youtube_js_1.YouTubeAdapter.searchAndPlay(query, fastPathGoal.entities.index || 1).catch(err => {
                        ...
                        browser_automator_js_1.BrowserAutomator.getInstance().navigate(navUrl).then(async () => {
                            await youtube_js_1.YouTubeAdapter.playResult(Math.floor(Math.random() * 3) + 1);
                        }).catch(() => { });
                    }
                }
                else if (fastPathGoal.suggestedTargetUrl) {
                    browser_automator_js_1.BrowserAutomator.getInstance().navigate(fastPathGoal.suggestedTargetUrl).catch(() => { });
                }
                else {
                    media_controller_js_1.MediaController.getInstance().play().catch(() => { });
                }
            }
                ...
                profiler.markTtsStart(ackText);
                this.speak(ackText).finally(() => profiler.markTtsEnd()).catch(() => { });
                youtube_js_1.YouTubeAdapter.playResult(index).catch(err => {
                ...
                profiler.markTtsStart(ackText);
                this.speak(ackText).finally(() => profiler.markTtsEnd()).catch(() => { });
                await media_controller_js_1.MediaController.getInstance().pause();
            }
          ...
          `);
            ...
            context_manager_js_1.ContextManager.getInstance().recordChainStep({
        ...
        const capabilityRoute = capability_router_js_1.CapabilityRouter.getInstance().route(goal);
        if (capabilityRoute.tier === 'NATIVE_SERVICE_CONNECTOR' && capabilityRoute.targetTool) {
            ...
            if (capabilityRoute.targetTool === 'youtube.play') {
                ...
                const playGraph = goal_graph_js_1.GoalGraph.createYouTubePlayGraph({
                    query: capabilityRoute.parameters?.query,
                    isRandom: capabilityRoute.parameters?.isRandom,
                    automator: browser_automator_js_1.BrowserAutomator.getInstance(),
                    adapter: youtube_js_1.YouTubeAdapter,
                    media: media_controller_js_1.MediaController.getInstance(),
                });
            ...
            try {
                ...
                const result = await service_connector_js_1.ConnectorRegistry.getInstance().executeServiceTool(capabilityRoute.targetTool, capabilityRoute.parameters || {});
                ...
                if (capabilityRoute.targetTool === 'calendar.today') {
                    if (Array.isArray(result) && result.length > 0) {
                        spokenResponse = `You have ${result.length} event${result.length > 1 ? 's' : ''} today: ` +
                            result.slice(0, 3).map((e) => `${e.summary} at ${new Date(e.start).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`).join(', ');
                    }
                    else {
                    ...
                    if (Array.isArray(result) && result.length > 0) {
                        spokenResponse = `Found ${result.length} file${result.length > 1 ? 's' : ''} in Google Drive: ` +
                            result.slice(0, 3).map((f) => f.name).join(', ');
                    }
                    else {
        ...
        if (interpreted.isCoherent === false || interpreted.isUncertain || interpreted.confidence < 0.6) {
            ...
            if (hasMeaningfulContent) {
                ...
                await browser_automator_js_1.BrowserAutomator.getInstance().navigate(searchUrl);
