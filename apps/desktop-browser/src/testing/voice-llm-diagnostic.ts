...
export async function runVoiceLlmDiagnosticSuite(): Promise<DiagnosticReport[]> {
  console.log('\n===============================================================');
  ...
  // ═════════════════════════════════════════════════════════════════════════
  {
    ...
    try {
      ...
      let spokenMessage = '';
      const origSpeak = (agentRuntime as any).speak;
      (agentRuntime as any).speak = async (msg: string) => {
        spokenMessage = msg;
        console.log(`[Diagnostic 1] Agent spoke apology: "${msg}"`);
      };

      await agentRuntime.handleUserCommand(rep.userInput);
      (agentRuntime as any).speak = origSpeak;
  ...
  // ═════════════════════════════════════════════════════════════════════════
  {
    ...
    try {
      ...
      const hasNavigationOrSearch = plan.steps.some(
        s => s.toolName.includes('navigate') || s.toolName.includes('type') || s.description.toLowerCase().includes('youtube')
      );
  ...
  // ═════════════════════════════════════════════════════════════════════════
  {
    ...
    try {
      ...
      const transcription = await WhisperBridge.transcribe(samples);
...
if (typeof window !== 'undefined') {
  (window as any).runVoiceLlmDiagnosticSuite = runVoiceLlmDiagnosticSuite;
}
