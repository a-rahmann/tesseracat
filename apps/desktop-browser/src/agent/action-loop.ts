...
import { AgentDecision, PlanStep } from './types.js';
...
export class ActionLoop {
  ...
  public async run(
    ...
    const checkpointManager = TaskCheckpointManager.getInstance();
    ...
    let stepNumber = 1;
    let lastActionInfo: { tool: string; args?: any; result?: string; error?: string } | undefined;
...
Output strictly valid JSON matching this schema:
{
  ...
  "confidence": number
      ...
      if (decision.reason || (decision as any).thought) {
        callbacks.onStatus(decision.reason || (decision as any).thought);
      }

      // Check for mission completion
      if (decision.isFinalStep || decision.tool === 'task.finish' || decision.type === 'complete') {
        const summary = decision.reason || (decision as any).thought || 'Mission completed successfully.';
      ...
      try {
        ...
        const recovery = await this.attemptIntelligentRecovery(
          activeTask,
          goal,
          stepNumber,
          tool.name,
  ...
  private async attemptIntelligentRecovery(
    ...
    callbacks: ActionLoopCallbacks,
    token: CancellationToken
  ): Promise<{ recovered: boolean; recoverySummary?: string }> {
    try {
      ...
      const compactSnap = AccessibilityTreeFormatter.toCompactString(freshSnapshot.elements);
      ...
      const replannedSteps = await planner.replan(
        ...
        failureReason,
        {
          ...
          availableTools: ToolRegistry.getInstance().listToolNames(),
        }
      );
      ...
      const recoveryTool = ToolRegistry.getInstance().getTool(recoveryStep.toolName);
      if (!recoveryTool) {
      ...
      const verified = await this.verifyRecoveryOutcome(
        freshSnapshot,
        postRecoverySnapshot,
        recoveryStep.toolName,
      ...
      if (verified) {
        ...
        TaskManager.getInstance().transitionState('EXECUTING', { currentActionDescription: recSummary });
   ...
   */
  private async verifyRecoveryOutcome(
    preSnapshot: PageSnapshot,
  ...
  private async waitForPageStabilization(perception: BrowserPerception, timeoutMs = 45000): Promise<void> {
  ...
  private async waitForAuthenticationSuccess(perception: BrowserPerception, timeoutMs = 60000): Promise<void> {
    const start = Date.now();
    while (Date.now() - start < timeoutMs) {
      const snap = await perception.getSnapshot();
