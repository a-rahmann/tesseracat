...
export class IntentEngine {
  ...
  private matchSocialDMs(text: string, rawText: string, cleanText: string, inNewTab: boolean): StructuredIntent | null {
    ...
    if (
      /(?:check|read|open|see|look\s+at)\s+(?:my\s+)?(?:dms?|messages?|texts?|inbox)/i.test(text) ||
  ...
  private matchAutofillForm(text: string, rawText: string, cleanText: string): StructuredIntent | null {
  ...
  private matchCoBrowsing(text: string, rawText: string, cleanText: string): StructuredIntent | null {
