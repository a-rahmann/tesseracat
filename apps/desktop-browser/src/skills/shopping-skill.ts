...
import { BrowserStateStore, SearchResultItem } from '../memory/browser-state-store.js';
import { TemporalMemory } from '../memory/temporal-memory.js';
import { OllamaGemmaModel } from '../ai/ollama-gemma.js';

export class ShoppingSkill implements Skill {
  public readonly name = 'ShoppingSkill';
  public readonly description = 'Autonomous e-commerce search, budget constraint filtering, and product comparison';
  ...
  public async execute(goal: string, context: SkillContext): Promise<SkillResult> {
    ...
    if (!itemQuery) itemQuery = 'mechanical keyboard';
    ...
    const productCandidates: SearchResultItem[] = [];
    let idx = 1;
    ...
    BrowserStateStore.getInstance().recordSearch(fullSearch, 'Amazon', productCandidates);
