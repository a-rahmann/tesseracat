...
import { UserMemoryStore } from '../services/user-memory.js';
import { BrowserAutomator } from '../services/browser-automator.js';

export class FormsSkill implements Skill {
  public readonly name = 'FormsSkill';
  public readonly description = 'Autonomous form input detection and local privacy-preserving autofill';
  ...
  public async execute(goal: string, context: SkillContext): Promise<SkillResult> {
    ...
    for (const box of textboxes) {
      ...
      if (label.includes('name') || label.includes('full')) fillVal = profile.fullName;
      else if (label.includes('email')) fillVal = profile.email || 'user@example.com';
      else if (label.includes('phone') || label.includes('mobile')) fillVal = profile.phone || '';
      else if (label.includes('street') || label.includes('address')) fillVal = profile.streetAddress;
      else if (label.includes('city')) fillVal = profile.city;
      else if (label.includes('state')) fillVal = profile.state;
      else if (label.includes('zip') || label.includes('postal') || label.includes('pin')) fillVal = profile.postalCode;
      else if (label.includes('country')) fillVal = profile.country;
