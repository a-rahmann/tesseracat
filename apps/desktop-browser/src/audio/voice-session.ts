...
import { resampleTo16k } from './resampler.js';
...
export interface VoiceSessionOptions {
  ...
  whisperTranscribe?: (pcm16k: Float32Array) => Promise<{ success: boolean; text?: string; error?: string }>;
}
...
export class VoiceSession {
  ...
  constructor(options: VoiceSessionOptions = {}) {
    ...
    this.wakeDetector = new WakeWordDetector({ enabled: false });
  ...
  public setState(newState: VoiceState, detail?: string): void {
  ...
  public setTranscriptionHandler(fn: (text: string) => void): void {
    this.options.onTranscriptionResult = fn;
  }

  public setWhisperCaller(fn: (pcm16k: Float32Array) => Promise<{ success: boolean; text?: string; error?: string }>): void {
      ...
      const { sampleRate } = await this.capture.start({
        onPcmChunk: async (chunk) => {
          if (this.state === 'listening-for-wake') {
            const pcm16k = await resampleTo16k(chunk, this.nativeSampleRate);
  ...
  public async stopRecordingAndTranscribe(): Promise<void> {
    ...
    try {
      ...
      } else if (typeof (window as any).tesseractNative?.whisperTranscribe === 'function') {
        response = await (window as any).tesseractNative.whisperTranscribe(pcm16k);
      }
  ...
  private handleWakeDetected(score: number): void {
  ...
  private async startWakeListening(): Promise<void> {
  ...
  private async stopCaptureAndReset(): Promise<void> {
    this.clearTimers();
    this.isCaptureActive = false;
