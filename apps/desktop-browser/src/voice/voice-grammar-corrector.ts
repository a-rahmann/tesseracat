...
export class VoiceGrammarCorrector {
  private static instance: VoiceGrammarCorrector | null = null;

  // Canonical platform dictionary (60+ platforms)
  private canonicalPlatforms: Record<string, string[]> = {
  ...
  private canonicalActions: Record<string, string[]> = {
  ...
  public static getInstance(): VoiceGrammarCorrector {
   ...
   */
  public soundex(s: string): string {
   ...
   * Runs in <1ms.
   */
  public correct(rawText: string): GrammarCorrectionResult {
      ...
      for (const alias of aliases) {
        ...
        if (regex.test(text)) {
          ...
          if (canonical === 'play' && (alias === 'be' || alias === 'k' || alias === 'c')) {
            ...
            if (!mediaFollower.test(text)) continue;
          ...
          if (canonical === 'cart' && alias === 'card') {
            ...
            if (!cartContext.test(text)) continue;
          ...
          if (canonical === 'tab' && alias === 'tap') {
            ...
            if (!tabContext.test(text)) continue;
          ...
          if (canonical === 'site' && alias === 'sight') {
            ...
            if (!siteContext.test(text)) continue;
          ...
          if (canonical === 'cache' && alias === 'cash') {
            ...
            if (!cacheContext.test(text)) continue;
    ...
    const homophoneRepairs: Array<{ pattern: RegExp; replacement: string; reason: string }> = [
      // Shopping / E-commerce
   ...
   */
  public correctArticles(text: string): { text: string; changes: CorrectionChange[] } {
    let result = text;
    const changes: CorrectionChange[] = [];
   ...
   */
  public deduplicateStutter(text: string): string {
   ...
   */
  public correctGrammarOnly(text: string): GrammarCorrectionResult {
   ...
   */
  private cleanAcousticArtifacts(text: string): string {
    return text
