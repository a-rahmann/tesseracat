...
export interface PromptContext {
  ...
  lastAction?: { tool: string; result?: string; error?: string };
  availableTools?: Array<{ name: string; description: string; parameters: string }>;
}
...
4. Output must be valid JSON specifying the next tool to execute.`;
  ...
  public static buildObservationActionPrompt(ctx: PromptContext): string {
    ...
    if (ctx.lastAction) {
      ...
      if (ctx.lastAction.result) prompt += `Result: ${ctx.lastAction.result.slice(0, 300)}\n`;
      if (ctx.lastAction.error) prompt += `Error: ${ctx.lastAction.error}\n`;
    }
